...
// we use devRunner to test our code during development
...
@Component // 相当于一个接口告诉spring boot去run这里面的代码
...
//        // 创建一个class的object。 Id 是null 因为是自动生成id，如果不是null那么就是update。但是如果写的id不存在 那么就会报错
...
//        OrderItemEntity orderItemEntity = orderItemRepository.findById(1L).get();
